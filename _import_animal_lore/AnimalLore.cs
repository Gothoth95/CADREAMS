...
namespace Server.SkillHandlers
{
    public class AnimalLore
    {
        ...
        {
            if (PetTrainingHelper.Enabled && m.HasGump(typeof(DashboardAnimalLoreGump)))
            {
        ...
        private class InternalTarget : Target
        {
			private static void SendGump(Mobile from, BaseCreature c)
			{
                ...
                if (PetTrainingHelper.Enabled && from is PlayerMobile)
                {
                    Timer.DelayCall(TimeSpan.FromSeconds(1), () =>
                        {
                            BaseGump.SendGump(new DashboardAnimalLoreGump((PlayerMobile)from, c));
                        });
                }
                else
                {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                // Check if the target is a living pet OR a shrunken pet statue
                else if (targeted is BaseCreature || targeted is IShrinkItem)
                {
                    BaseCreature c = null;

                    if (targeted is BaseCreature)
                        c = (BaseCreature)targeted;
                    else if (targeted is IShrinkItem)
                        c = ((IShrinkItem)targeted).ShrunkenPet;

                    if (c != null && !c.IsDeadPet)
                    {
                        // ... rest of the standard Animal Lore code ...
                        if (c.Body.IsAnimal || c.Body.IsMonster || c.Body.IsSea || c is BaseHire || c is BaseMount)
                        {
