        ...
        public override bool OnMoveOver(Mobile m)
        {
            if (m is BaseCreature)
            {
                    ...
                    && profile != null && profile.HasBegunTraining && profile.TrainingProgress < profile.TrainingProgressMax)
                {
                    ...
                    {
                        bc.PublicOverheadMessage(Server.Network.MessageType.Regular, 0x59, false, "*" + bc.Name + " surges with battle experience and is ready to train!*");

                        var gump = bc.ControlMaster.FindGump<DashboardAnimalLoreGump>();
