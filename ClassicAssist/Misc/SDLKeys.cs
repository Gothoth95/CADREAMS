...
namespace ClassicAssist.Misc
{
    public static class SDLKeys
    {
        ...
        // Italian (1040/2064): matches Windows VK→WPF Key for IT QWERTY (see auto keymap logs).
        // Used when SDL delivers Unicode keycodes; do not override keys VkKeyScan already maps correctly.
        private static readonly Dictionary<int, Key> ItalianLocaleKeyMap = new Dictionary<int, Key>
        {
            { '\'', Key.OemOpenBrackets },
            { '?', Key.OemOpenBrackets },
            { ';', Key.OemComma },
        ...
        private static readonly Dictionary<int, Dictionary<int, Key>> _localeKeyMap =
            new Dictionary<int, Dictionary<int, Key>>
            {
                ...
                { 1040 /* Italian (Italy) */, ItalianLocaleKeyMap },
                { 2064 /* Italian (Switzerland) */, ItalianLocaleKeyMap }
            };
        ...
        public static Key SDLKeyToKeys( int sdlKey )
        {
            ...
            // Fallback for locale-specific printable keys (e.g. Italian accent/OEM keys).
            // SDL can emit unicode keycodes that are not present in INTERNAL_keyMap.
            if ( TryResolveLocalizedPrintableKey( sdlKey, out Key localizedKey ) )
            {
                return localizedKey;
            }
        ...
        /// <summary>
        ///     Maps a typed character to a WPF <see cref="Key" /> using the current WPF input language (hotkey TextBox).
        /// </summary>
        public static bool TryKeyFromCharacter( char character, out Key key )
        {
            return TryKeyFromCharacter( character, IntPtr.Zero, out key );
        }

        /// <summary>
        ///     Maps a character using a specific keyboard layout (HKL). Use this from background threads:
        ///     <see cref="InputLanguageManager.Current" /> is null fuori dal thread UI e
        ///     <see cref="InputLanguage.FromCulture" /> può restituire null.
        /// </summary>
        public static bool TryKeyFromCharacter( char character, IntPtr keyboardLayoutHandle, out Key key )
        {
            key = Key.None;
            ...
            IntPtr layoutHandle = keyboardLayoutHandle;

            if ( layoutHandle == IntPtr.Zero )
            {
                CultureInfo culture = InputLanguageManager.Current?.CurrentInputLanguage;
                InputLanguage inputLanguage = culture != null ? InputLanguage.FromCulture( culture ) : null;
                layoutHandle = inputLanguage?.Handle ?? IntPtr.Zero;

                if ( layoutHandle == IntPtr.Zero )
                {
                    layoutHandle = InputLanguage.CurrentInputLanguage?.Handle ?? IntPtr.Zero;
                }

                if ( layoutHandle == IntPtr.Zero )
                {
                    layoutHandle = ResolveEngineKeyboardLayoutHandle();
                }
            }

            return TryVkKeyScanToKey( character, layoutHandle, out key );
        }

        private static IntPtr ResolveEngineKeyboardLayoutHandle()
        {
            if ( Engine.KeyboardLayoutId != 0 )
            {
                return new IntPtr( Engine.KeyboardLayoutId );
            }

            return InputLanguage.CurrentInputLanguage?.Handle ?? IntPtr.Zero;
        }

        private static bool TryVkKeyScanToKey( char character, IntPtr layoutHandle, out Key key )
        {
            key = Key.None;
            short vkScan = VkKeyScanEx( character, layoutHandle );
            ...
            int virtualKey = vkScan & 0xFF;
            key = KeyInterop.KeyFromVirtualKey( virtualKey );

            return key != Key.None;
        }

        private static bool TryResolveLocalizedPrintableKey( int sdlKey, out Key key )
        {
            key = Key.None;

            // SDL scancode keycodes are handled by INTERNAL_keyMap.
            if ( ( sdlKey & SDLK_SCANCODE_MASK ) == SDLK_SCANCODE_MASK )
            {
                return false;
            }

            if ( sdlKey <= 0 || sdlKey > char.MaxValue )
            {
                return false;
            }

            return TryVkKeyScanToKey( (char) sdlKey, ResolveEngineKeyboardLayoutHandle(), out key );
        }
